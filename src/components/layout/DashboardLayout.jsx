...
function DashboardLayout({ children }) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-950">
      ...
      <aside className={`fixed left-0 top-0 z-40 h-full w-64 border-r border-white/5 bg-slate-900/50 backdrop-blur-xl transition-transform duration-300 md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:w-16'}`}>
        <div className="flex h-16 items-center justify-between border-b border-white/5 px-4">
          <Link to="/" className="text-lg font-bold bg-gradient-to-r from-violet-400 to-fuchsia-400 text-transparent bg-clip-text whitespace-nowrap md:block" style={sidebarOpen ? {} : { display: 'none' }}>CineVerse</Link>
